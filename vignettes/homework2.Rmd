...
&=(QR)(R^TR)^{-1}R^TQ^T \,\,\,\text{since}\,\, Q^{T}Q = I\\
&=(QR)R^TQ^T \,\,\,\text{since}\,\, R^{T}R = I\\
&=QQ^T \,\,\,\text{since} R^{T}R = I
\end{aligned}
\]
...
\text{det}(X^TX) &= \text{det}((QR)^TQR)\\
          ...
          &= \text{det}(R)\text{det}(R) \,\,\,\text{since}\,\,R^T = R\\ 
          &= [\text{det}(R)]^2
\end{aligned}
\]
...
Here I write an R function that takes as input an $n$ dimensional numeric vector $\bf{\mu}$ and a $n \times n$ positive definite matrix $\bf{\Sigma}$ and returns $N$ realizations from a multivariate normal distribution $\text{MVN}(\bf{\mu}, \bf{\Sigma})$ using Cholesky decomposition. Written code and documentation for the function, which I call `multvnorm()`, is found [here](https://github.com/miningroup/your-stat-230-r-package-davymwax/blob/3ca9ddc9b6319547c0b8852d1f079e9bada7e357/R/multvnorm.R).

I create a test case with $n = 4$ and $N = 100$ and use the sample mean and sample covariance matrices to validate my function. I also compare my function to the existing multivariate normal simulator `mvrnorm()` implemented in the [MASS package](https://cran.r-project.org/web/packages/MASS/index.html).
...
The results of benchmarking the computational efficiency of both implementations suggest that QR decomposition (done in a minimum time of $219\mu s$) is about 3 times as fast as Cholesky decomposition (done in a minimum time of $578\mu s$). 
