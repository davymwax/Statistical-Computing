...
Substituting $x^{\text{curr}}$ and $x^{\text{prop}} = x^{\text{curr}}\times \text{exp}(y)$ into $a_{\text{curr}, \text{prop}}$, this the Rosenbluth-Hastings ratio simplifies to:

\[
x^{\text{curr}}\text{exp}(3y - \text{exp}(y))\text{exp}\bigg(\frac{-(y-1)^2 + (\text{log}(x^{\text{curr}})+y - 1)^2)}{2\text{exp}(\sigma^2)} \bigg)
\]
...
hist(mcmc_output[,"state"], 
     xlab="MCMC State", 
     main="Target Distribution Histogram")
```
